...
function M.ask(display_mode, mode, user_prompt)
	display_mode = display_mode or M.config.default_window
	local bufnr = vim.api.nvim_get_current_buf()

	if user_prompt then
		M.open_with_prompt(display_mode, bufnr, user_prompt)
		return
	end

	vim.ui.input({ prompt = 'Ask sven: ' }, function(input)
		M.open_with_prompt(display_mode, bufnr, input or '')
	end)
end

function M.open_with_prompt(display_mode, bufnr, user_prompt)
